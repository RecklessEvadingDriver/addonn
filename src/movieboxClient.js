...
class MovieBoxClient {
  ...
  clientProfile(name = 'mboxIn') {
    return CLIENT_PROFILES[name] ?? CLIENT_PROFILES.mboxIn;
  }

  clientInfo(profileName = 'mboxIn', overrides = {}) {
    const profile = this.clientProfile(profileName);
  ...
  signedHeaders(method, url, body = null, options = {}) {
    ...
    const profileName = options.profile ?? 'mboxIn';
    const profile = this.clientProfile(profileName);
    const { brand, model } = randomBrandModel();
    return {
      'user-agent': `${profile.packageName}/${profile.versionCode} (Linux; U; Android ${profile.osVersion}; ${profile.locale}; ${model}; Build/${profile.build}; Cronet/${profile.cronet})`,
      accept,
      ...
      'x-client-info': this.clientInfo(profileName, options.clientInfo),
      'x-client-status': '0',
      'x-play-mode': profile.extraInfo['X-Play-Mode'] ?? '2',
  ...
  async request(method, url, { body = null, ...options } = {}) {
    ...
    if (!response.ok && response.status === 403 && text.includes('Service not available in current region') && !options.disableProfileFallback) {
      const nextProfile = (options.profile ?? 'mboxIn') === 'mboxIn' ? 'oneroomUs' : 'mboxIn';
      return this.request(method, url, { ...options, body: textBody, profile: nextProfile, disableProfileFallback: true });
    }
    if (!response.ok) {
